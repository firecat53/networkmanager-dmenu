...
def run_dmenu(command, input_text=None):
    """Run a dmenu command and handle errors properly.

    Args:
        command: list of command arguments
        input_text: optional string to pass as stdin

    Returns:
        subprocess.CompletedProcess result

    Raises:
        SystemExit: if the dmenu command fails with an error
    """
    ...
    if result.returncode != 0 and result.stderr:
        error_msg = f"dmenu command failed: {result.stderr.strip()}"
        print(error_msg, file=sys.stderr)
        notify(error_msg, urgency="critical")
...
def choose_adapter(client):
    ...
    sel = run_dmenu(dmenu_cmd("CHOOSE ADAPTER:"), device_names).stdout
    if not sel.strip():
        sys.exit()
    ...
    if pinentry:
        ...
        if out:
            ...
            if res and res[0].startswith("D "):
                pin = res[0].split("D ")[1]
        return pin
    return run_dmenu(dmenu_cmd("Passphrase"), "").stdout
...
def delete_connection():
    ...
    sel = run_dmenu(dmenu_cmd("CHOOSE CONNECTION TO DELETE:"), conn_names).stdout
    if not sel.strip():
        sys.exit()
